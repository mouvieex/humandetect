...
uploaded_file = st.file_uploader("Zagruzite foto", type=['jpg', 'jpeg', 'png'])
...
if uploaded_file:
    ...
    if st.button("🔍 Nayti lyudey", type="primary"):
        with st.spinner("Obrabotka..."):
            try:
                arr = np.array(img)
                ...
                st.image(res_img, caption=f"✅ Nashel: {cnt}", use_container_width=True)
                
                buf = BytesIO()
                res_img.save(buf, format='JPEG', quality=60)
                buf.seek(0)
                
                st.download_button("📥 Skachat", buf.getvalue(), "result.jpg", "image/jpeg")
